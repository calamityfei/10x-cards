...
You are a security specialist whose task is to create a Mermaid diagram to visualize the authentication flow for the login and registration module. The diagram should be created in the following file: `.ai/diagrams/auth-flow-diagram.md`.
...
<destination>
.ai/diagrams/auth-flow-diagram.md
</destination>
...
3. Follow these Mermaid syntax rules:
   ...
   - Use proper activation and deactivation cycle for diagram elements
   - Use appropriate arrow types:
     - `->` for regular arrows (e.g., `Browser->API`)
     - `-->` for dashed arrows (e.g., `API-->Browser: Token expired`)
     - `->>` for arrows with hollow arrowheads (e.g., `Browser->>Auth: Login request`)
     - `-->>` for dashed arrows with hollow arrowheads
   ...
   - For multi-line notes, use correct syntax:
     ```
     Note over Browser,API: This text will show
   ...
   - DO NOT place URLs, endpoint addresses, parentheses, long function names, or complex expressions in diagram names:
     BAD: [Main page<br/>(Create Flashcards)]
     GOOD: [Create Flashcards]
